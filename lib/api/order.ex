...
defmodule Zalora.Order do
  ...
  https://sellercenter-api-staging.zalora.com.ph/docs/#/Orders/GET_v2-orders-shipment-providers
  """
  def get_shipment_providers(opts \\ []) do
    with {:ok, client} <- Client.new(opts),
         {:ok, items} when is_list(items) <- Client.get(client, "/v2/orders-shipment-providers") do
      {:ok, items}
    else
      {:ok, data} ->
        {:error, data}
